...
/// has — so Settings can warn when Rainy Day's own idle timeout is set at
...
/// for Rainy Day's own idle timeout to lose a race against.
///
...
///
/// `AppDelegate`'s idle tick will not start the saver while macOS's own
/// screen saver is running or the display is asleep. So the number
/// Rainy Day's idle timeout has to beat is the soonest of *those*. A lock
/// that follows later changes nothing: by then the saver could not have
/// started anyway.
enum SystemScreenLockSettings {
    ...
    /// unreadable) — nothing for Rainy Day's own idle timeout to lose a
    /// race against at all.
